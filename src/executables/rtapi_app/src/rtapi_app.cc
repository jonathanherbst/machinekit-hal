...
typedef int (*hal_call_usrfunct_t)(const char *name, const int argc,
                                   const char **argv, int *ureturn);
static hal_call_usrfunct_t call_usrfunct;

typedef int (*halg_foreach_t)(bool use_hal_mutex, foreach_args_t *args,
...
static void stderr_rtapi_msg_handler(msg_level_t level, const char *fmt,
                                     va_list ap);
static int record_instparms(std::shared_ptr<Module> mi);

static void configure_flavor(machinetalk::Container &pbreply,
                             std::shared_ptr<Module> mi)
{
                       ...
                       machinetalk::Container &pbreply)
{
    ...
    default:
        note_printf(pbreply, "%s: Invalid type character `%c'\n",
                          ...
                          machinetalk::Container &pbreply)
{
    for (int i = 0; i < args.size(); i++) {
        ...
        if (!item_type || !*item_type) {
            if (mi->errmsg) note_printf(pbreply, "BUG: %s:", mi->errmsg);
            note_printf(pbreply,
        ...
        if (item_type_string.size() > 1) {
            int a, b;
            char item_type_char;
            int r = sscanf(item_type_string.c_str(), "%d-%d%c", &a, &b,
                           &item_type_char);
            if (r != 3) {
                note_printf(pbreply,
                            "Unknown parameter `%s'"
            ...
            int result =
                do_one_item(item_type_char, s, param_value, item, 0, pbreply);
            if (result != 0) return result;
        }
    }
    return 0;
...
static void separate_kv(pbstringarray_t &kvpairs, pbstringarray_t &leftovers,
                        const pbstringarray_t &args)
{
    ...
    for (int i = 0; i < args.size(); i++) {
        ...
        if (std::equal(prefix.begin(), prefix.end(), s.begin())) extra = true;
                          ...
                          pbstringarray_t args, machinetalk::Container &pbreply)
{
    ...
    if (modules.count(comp) == 0) {
        // if newinst via halcmd, it should have been automatically loaded
        // already
        note_printf(pbreply, "newinst: component '%s' not loaded",
                    comp.c_str());
        return -1;
    }
    ...
    if (retval < 0) {
        note_printf(pbreply, "passing args for '%s' failed: '%s'",
                    instname.c_str(), s.c_str());
        return retval;
    }
    rtapi_print_msg(RTAPI_MSG_DBG, "%s: instargs='%s'\n", __FUNCTION__,
                    s.c_str());
                           ...
                           machinetalk::Container &pbreply)
{
    ...
    int ureturn = 0;
    retval = call_usrfunct(func.c_str(), args.size(), argv, &ureturn);
    if (argv) free(argv);
    if (retval == 0) retval = ureturn;
...
static int init_actions(int instance)
{
    ...
    if (call_usrfunct == NULL) {
        rtapi_print_msg(RTAPI_MSG_ERR, "cant resolve 'hal_call_usrfunct' in "
                                       "hal_lib - version problem?");
        if (hallib->errmsg)
            rtapi_print_msg(RTAPI_MSG_ERR, "dlsym(hal_call_usrfunct): '%s'",
                            hallib->errmsg);
        return -1;
    }
    return 0;
...
static int attach_global_segment()
{
    ...
    shm_common_init();
    do {
        retval = shm_common_new(globalkey, &size, rtapi_instance_loc,
                                (void **)&global_data, 0);
        if (retval < 0) {
            tries--;
            if (tries == 0) {
                syslog_async(LOG_ERR,
                             "rtapi_app:%d: ERROR: cannot attach global "
                             "segment key=0x%x %s\n",
    ...
    while (global_data->magic != GLOBAL_READY) {
        ...
        if (tries == 0) {
            syslog_async(LOG_ERR,
                         "rtapi_app:%d: ERROR: global segment magic not "
                         "changing to ready: magic=0x%x\n",
...
static int rtapi_request(zloop_t *loop, zsock_t *socket, void *arg)
{
    ...
    if (request_frame == NULL) {
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "rtapi_request(): NULL zframe_t 'request_frame' passed");
        return -1;
    }
    ...
    if (!pbreq.ParseFromArray(zframe_data(request_frame),
        ...
        rtapi_print_msg(RTAPI_MSG_ERR, "cant decode request from %s (size %zu)",
                        origin ? origin : "NULL", zframe_size(request_frame));
        zmsg_destroy(&r);
        return 0;
    }
    if (z_debug) {
    ...
    case machinetalk::MT_RTAPI_APP_PING:
        ...
        snprintf(buffer, sizeof(buffer), "pid=%d flavor=%s gcc=%s git=%s",
                 getpid(), (*flavor_name_ptr)(NULL), __VERSION__,
                 GIT_BUILD_SHA);
    ...
    case machinetalk::MT_RTAPI_APP_NEWINST:
        ...
        assert(pbreq.rtapicmd().has_instance());
        pbreply.set_retcode(do_newinst_cmd(
            pbreq.rtapicmd().instance(), pbreq.rtapicmd().comp(),
            pbreq.rtapicmd().instname(), pbreq.rtapicmd().argv(), pbreply));
        break;
    ...
    case machinetalk::MT_RTAPI_APP_DELINST:
        ...
        assert(pbreq.rtapicmd().has_instname());
        assert(pbreq.rtapicmd().has_instance());
        pbreply.set_retcode(do_delinst_cmd(
            pbreq.rtapicmd().instance(), pbreq.rtapicmd().instname(), pbreply));
        break;
    ...
    case machinetalk::MT_RTAPI_APP_UNLOADRT:
        ...
        pbreply.set_retcode(do_unload_cmd(pbreq.rtapicmd().instance(),
                                          pbreq.rtapicmd().modname(), pbreply));
        break;
    ...
    case machinetalk::MT_RTAPI_APP_NEWTHREAD:
        ...
        strncpy(args.cgname, pbreq.rtapicmd().cgname().c_str(),
                RTAPI_LINELEN - 1);
    ...
    default:
        rtapi_print_msg(RTAPI_MSG_ERR, "unkown command type %d)",
                        (int)pbreq.type());
        zmsg_destroy(&r);
        return 0;
    }
    ...
    for (int i = 0; i < pbreply.note_size(); i++) {
        rtapi_print_msg(pbreply.retcode() ? RTAPI_MSG_ERR : RTAPI_MSG_DBG, "%s",
 ...
 * in Debian Stretch and Ubuntu Bionic */
    ...
    if (reply == NULL) {
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "rtapi_request(): NULL zframe_t 'reply' passed");
        return -1;
    }
    ...
    if (!pbreply.SerializeWithCachedSizesToArray(zframe_data(reply))) {
        ...
        if (zframe_send(&reply, socket, 0)) {
            rtapi_print_msg(
                RTAPI_MSG_ERR, "cant serialize to %s (type %d size %zu)",
                origin ? origin : "NULL", pbreply.type(), zframe_size(reply));
        }
    }
    free(origin);
    zmsg_destroy(&r);
...
static void sigaction_handler(int sig, siginfo_t *si, void *uctx)
{
    ...
    case SIGXCPU:
        ...
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "rtapi_app: BUG: SIGXCPU should be handled in RTAPI");
        // NB: fall through

    default:
        rtapi_print_msg(
            RTAPI_MSG_ERR,
            "signal %d - '%s' received, dumping core (current dir=%s)", sig,
            strsignal(sig), get_current_dir_name());
        backtrace("", "rtapi_app", btprint, 3);
        if (global_data) global_data->rtapi_app_pid = 0;
...
static int s_handle_signal(zloop_t *loop, zmq_pollitem_t *poller, void *arg)
{
    ...
    if (s != sizeof(struct signalfd_siginfo)) {
        rtapi_print_msg(RTAPI_MSG_ERR, "BUG: read(signal_fd): %s",
                        strerror(errno));
        return 0;
    }
    ...
    case SIGKILL:
    case SIGTERM:
        rtapi_print_msg(RTAPI_MSG_INFO, "signal %d - '%s' received, exiting",
    ...
    default:
        ...
        rtapi_print_msg(RTAPI_MSG_ERR,
                        "BUG: unhandled signal %d - '%s' received\n",
                        fdsi.ssi_signo, strsignal(fdsi.ssi_signo));
    }
    return 0;
}

static int s_handle_timer(zloop_t *loop, int timer_id, void *args)
{
    (void)loop;
    (void)timer_id;
    (void)args;
...
static int mainloop(size_t argc, char **argv)
{
    ...
    if ((retval = attach_global_segment()) != 0) {
        syslog_async(LOG_ERR,
                     "%s: FATAL - failed to attach to global segment\n",
                     argv[0]);
        exit(retval);
    }
    ...
    if (trap_signals) {
        signal_fd = setup_signals(sigaction_handler, SIGINT, SIGQUIT, SIGKILL,
                                  SIGTERM, -1);
        assert(signal_fd > -1);
    }
    ...
    if ((remote || z_uri) && interfaces) {
        ...
        // rtapi_print_msg(RTAPI_MSG_INFO, "rtapi_app:
        // ifpref='%s'\n",interfaces);
        if (parse_interface_prefs(interfaces, ifname, ip, NULL) == 0) {
            rtapi_print_msg(RTAPI_MSG_INFO,
                            "rtapi_app: using preferred interface %s/%s\n",
                            ifname, ip);
            ipaddr = strdup(ip);
        } else {
            rtapi_print_msg(
                RTAPI_MSG_ERR,
                "rtapi_app: INTERFACES='%s'"
        ...
        if ((z_port = zsock_bind(z_command, z_uri)) == -1) {
            ...
            rtapi_print_msg(RTAPI_MSG_DBG,
                            "rtapi_app: command RPC socket on '%s'\n",
                            z_uri_dsn);
        }
    }
#endif
    { // always bind the IPC socket
        char uri[RTAPI_LINELEN];
        snprintf(uri, sizeof(uri), ZMQIPC_FORMAT, RUNDIR, rtapi_instance_loc,
                 "runtime" , service_uuid);
    ...
    if (remote) {
        if (!(av_loop = avahi_czmq_poll_new(z_loop))) {
            rtapi_print_msg(RTAPI_MSG_ERR,
                            "rtapi_app:%d: zeroconf: "
                            "Failed to create avahi event loop object.",
                            rtapi_instance_loc);
            return -1;
        } else {
            char name[255];
            snprintf(name, sizeof(name), "RTAPI service on %s pid %d", ipaddr,
                     getpid());
            rtapi_publisher = zeroconf_service_announce(
                name, MACHINEKIT_DNSSD_SERVICE_TYPE, RTAPI_DNSSD_SUBTYPE,
                z_port, (char *)z_uri_dsn, service_uuid, process_uuid_str,
                "runtime", NULL, av_loop);
            if (rtapi_publisher == NULL) {
                rtapi_print_msg(
                    RTAPI_MSG_ERR,
                    "rtapi_app:%d: failed to start zeroconf publisher",
                    rtapi_instance_loc);
                return -1;
            }
        }
    ...
    // report success
    rtapi_print_msg(RTAPI_MSG_INFO,
                    "rtapi_app:%d ready flavor=%s gcc=%s git=%s",
                    rtapi_instance_loc, (*flavor_name_ptr)(NULL), __VERSION__,
                    GIT_BUILD_SHA);
    ...
    rtapi_print_msg(RTAPI_MSG_INFO, "exiting mainloop (%s)\n",
...
static int configure_memory(void)
{
    ...
    if (use_drivers || (*flavor_feature_ptr)(NULL, FLAVOR_DOES_IO)) {
        ...
        if (mlockall(MCL_CURRENT | MCL_FUTURE)) {
            rtapi_print_msg(RTAPI_MSG_WARN, "mlockall() failed: %d '%s'\n",
...
static int harden_rt()
{
    ...
    if (prctl(PR_SET_DUMPABLE, 1) < 0)
        rtapi_print_msg(RTAPI_MSG_WARN,
                        "prctl(PR_SET_DUMPABLE) failed: no core dumps will be "
                        "created - %d - %s\n",
                        errno, strerror(errno));
    FILE *fd;
    if ((fd = fopen("/proc/sys/fs/suid_dumpable", "r")) != NULL) {
        ...
        if ((fscanf(fd, "%d", &flag) == 1) && (flag == 0)) {
            rtapi_print_msg(RTAPI_MSG_WARN,
                            "rtapi:%d: cannot create core dumps - "
                            "/proc/sys/fs/suid_dumpable contains 0",
                            rtapi_instance_loc);
            rtapi_print_msg(
                RTAPI_MSG_WARN,
    ...
    if (getrusage(RUSAGE_SELF, &rusage)) {
        ...
        if (atexit(exit_handler)) {
            rtapi_print_msg(RTAPI_MSG_WARN, "atexit() failed: %d '%s'\n", errno,
                            strerror(errno));
        }
    }
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        int option_index = 0;
        int curind = optind;
        c = getopt_long(argc, argv, "ShH:m:I:f:r:U:NFdR:n:i:sz", long_options,
                        &option_index);
        if (c == -1) break;
        ...
        case 'i': inifile = strdup(optarg); break;

        case 'I': rtapi_instance_loc = atoi(optarg); break;

        case 'f':
            strncpy(flavor_name_opt, optarg, MAX_FLAVOR_NAME_LEN - 1);
            break;

        case 'U': z_uri = optarg; break;

        case 'n': interfaces = strdup(optarg); break;

        case 'R': service_uuid = strdup(optarg); break;
        case 's': option |= LOG_PERROR; break;
        case 'z': z_debug = 1; break;
        case '?':
            if (optopt)
                fprintf(stderr, "bad short opt '%c'\n", optopt);
            else
                fprintf(stderr, "bad long opt \"%s\"\n", argv[curind]);
            // usage(argc, argv);
            exit(1);
            break;

        default: usage(argc, argv); exit(0);
        }
    }
    ...
    if (trap_signals && (getenv("NOSIGHDLR") != NULL)) trap_signals = false;
    ...
    if (service_uuid == NULL) {
        fprintf(stderr, "rtapi: no service UUID (-R <uuid> or environment "
                        "MKUUID) present\n");
        exit(-1);
    }
...
static int record_instparms(std::shared_ptr<Module> mi)
{
    ...
    for (i = 0; i < symbols.size(); i++) {
        ...
        if (ip.rfind(pat) == 0) {
            ...
            if (strlen(*type) == 1) {
                ...
                case 's':
                    s = **(char ***)addr;
                    snprintf(buffer, sizeof(buffer), "%s=\"%s\"", ip.c_str(),
                             s);
                    *iparm.Add() = buffer;
                    break;
                default:
    ...
    rtapi_print_msg(RTAPI_MSG_DBG, "%s default iparms: '%s'", mi->name.c_str(),
